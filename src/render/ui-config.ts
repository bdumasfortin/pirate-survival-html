import { CAMERA_ZOOM as CAMERA_ZOOM_VALUE } from "../game/config";

export const UI_FONT = "Zain";
export const CAMERA_ZOOM = CAMERA_ZOOM_VALUE;

export const INVENTORY_SLOT_SIZE = 52;
export const INVENTORY_SLOT_GAP = 10;
export const INVENTORY_BAR_PADDING = 18;
export const INVENTORY_BAR_MARGIN = 12;
export const INVENTORY_CORNER_RADIUS = 12;

export const EQUIPMENT_SLOT_SIZE = 40;
export const EQUIPMENT_SLOT_GAP = 8;
export const EQUIPMENT_SLOT_RADIUS = 10;
export const EQUIPMENT_GRID_COLUMNS = 3;
export const EQUIPMENT_GRID_ROWS = 2;

export const HUD_MARGIN = 18;
export const BAR_WIDTH = 220;
export const BAR_HEIGHT = 14;

export const CRAFT_TILE_SIZE = 72;
export const CRAFT_TILE_GAP = 14;
export const CRAFT_TILE_RADIUS = 14;
export const CRAFT_PANEL_PADDING = 18;
export const CRAFT_PANEL_RADIUS = 16;
export const CRAFT_PANEL_WIDTH = 240;
export const CRAFT_PANEL_GAP = 16;
export const CRAFT_PANEL_SECTION_GAP = 10;
export const CRAFT_INGREDIENT_ROW_HEIGHT = 22;
export const CRAFT_BUTTON_WIDTH = 120;
export const CRAFT_BUTTON_HEIGHT = 30;
export const CRAFT_BUTTON_RADIUS = 10;

export const ACTION_PROMPT_FONT_SIZE = 18;
export const ACTION_PROMPT_PADDING_X = 16;
export const ACTION_PROMPT_PADDING_Y = 8;

