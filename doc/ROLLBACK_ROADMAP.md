...
- [x] Add frame counter to the sim and store snapshots each fixed update.
- [x] Add rollback restore path (rewind to frame N).

## 4) Resimulation using buffered inputs
- [x] After a rollback restore, re-run updates from frame N to current frame.
- [x] Ensure inputs are loaded from the input buffer for each re-sim frame.

## 5) Input sync scaffolding (local + remote)
- [x] Define per-player input frame format (local now, ready for remote).
- [x] Add hooks to merge remote inputs into the frame buffer.
- [x] Add basic late-input handling (trigger rollback + resim).
- [x] Add player index mapping and per-player input queues.

## 6) Validation + determinism checks
- [x] Add a determinism smoke test (same seed + same inputs => same hash).
